...
**Archon pack**:
A second runner of the same Ticket DAG and Git contract, started on a Target with `archon workflow run`. Not the Orchestrator. Not a Run. Source is this repository's `.archon/workflows/ticket-dag/`, copied to `~/.archon/workflows/` (global). Pack bun TypeScript scripts reimplement the Git contract; they do not import this repository's TypeScript. Public drain is `ticket-dag-drain`: leftover rematch, then a loop of startable Tickets, then one read-only review session on Main. Each Ticket is two script nodes (implement, then conflict if settle is resolve) — two Pi SDK sessions, not parent/child, cwd the Ticket Worktree, not Archon `prompt:` and not a spawned `pi` CLI. After the loop, bun pastes `git diff <drain-start>...HEAD` into a Pi session whose tools are only `read` / `grep` / `find` / `ls`, writes `$ARTIFACTS_DIR/review.md`, and exits 0 (advisory; does not stamp Tickets). Drain may run more than one Ticket at a time; merge onto Main stays serial under the pack's lock. Inspect is Archon's run records, not `orchestrator inspect`. Mutual exclusion with a CLI Run is convention. Stop is `archon workflow cancel`; leftover rematch is the next drain's first node. There is no pack retry or recover workflow.
_Avoid_: calling the pack the Orchestrator; wrapping this CLI as an Archon node; static YAML nodes per Ticket; Archon `isolation: worktree` as the Ticket Worktree; importing `src/` or `dist/` from this repository; writing Orchestrator Run records from the pack; nested `archon workflow run` per Ticket; Archon `prompt:` for implement/conflict; Python pack scripts; Claude as the pack agent; spawning `pi -p`; two-axis review inside implement; `/code-review` as the unattended review entry; failing the drain on review findings
